...
`Quality policy` installs the locked npm tools, tests the quality runner and
required-job classifier, then runs the complete repository quality policy. That
policy checks style, whitespace, analyzers, direct and transitive NuGet
vulnerabilities, outdated NuGet packages, Markdown and npm vulnerabilities.

`Release build and tests` restores, builds and tests the full solution in Release
configuration. `Build (x64)`, `Build (x86)` and `Build (ARM64)` compile the WinUI
...
tests and the complete architecture matrix all report `success`. Failure,
...
summary. After the first representative pull request and `main` run, record API
