...
function Get-CoverageExclusionViolations([string[]]$AssemblyPath, [string[]]$TrustedGeneratedMembers = @()) {
    ...
    foreach ($path in $AssemblyPath) {
        ...
        foreach ($type in $assembly.GetTypes()) {
            ...
            $typeIsTrustedGenerated = $type.FullName -eq 'AutoGeneratedProgram'
            foreach ($attribute in $typeAttributes) {
                if ($attribute.AttributeType.Name -in 'ExcludeFromCodeCoverageAttribute', 'ExcludeFromCoverageAttribute' -and !$typeIsTrustedGenerated) {
